    ...
    <main className="w-full grow pt-18">
      <section className="py-10 mt-3 md:py-16 bg-secondary">
        <div className="max-w-[1400px] mx-auto px-4 text-center">
          <h1 className="text-3xl font-bold mb-4">{t("catalog.OUR COLLECTION")}</h1>
      ...
      <section className="max-w-[1400px] mx-auto px-5 py-8 flex gap-6">
        ...
        <aside className="w-64 pr-4">
          <h2 className="font-semibold text-lg mb-2">{t("catalog.Categories")}</h2>
          <ul className="space-y-1">
            {[{ label: t("catalog.View All Products"), value: "" }, { label: "T-Shirts", value: "1" }, { label: "Shorts", value: "2" }, { label: "Suits", value: "3" }].map((cat) => (
              <li key={cat.value}>
                <button
                  onClick={() => setSelectedCategory(cat.value)}
                  className={`block w-full text-left px-2 py-1 rounded hover:bg-accent ${
                    selectedCategory === cat.value ? "bg-primary text-white" : ""
                  }`}
                >
                  {cat.label}
                </button>
          ...
          <h2 className="font-semibold text-lg mt-6 mb-2">{t("catalog.Sizes")}</h2>
          <button
            onClick={() => setSelectedSize("44-52")}
            className={`px-3 py-1 rounded border ${selectedSize === "44-52" ? "bg-primary text-white" : ""}`}
          >
            44–52
          </button>

          <h2 className="font-semibold text-lg mt-6 mb-2">{t("catalog.Colors")}</h2>
              ...
              products[0].colors.map((colorObj, idx) => (
                <button
                  ...
                  className={`px-3 py-1 border rounded ${
                    selectedColor === colorObj.color_en ? "bg-primary text-white" : ""
                  }`}
                >
                  {colorObj.color_en}
                </button>
          ...
          <button onClick={handleClearFilters} className="text-red-500 mt-4 underline">
        ...
        <div className="flex-1">
          <div className="flex justify-end mb-4">
            <label className="text-sm font-medium mr-2">{t("catalog.Sort by")}:</label>
            <select
              className="border px-2 py-1 rounded"
              value={sortOrder}
              onChange={(e) => setSortOrder(e.target.value)}
            >
              <option value="lowToHigh">{t("catalog.Price: Low to High")}</option>
              <option value="highToLow">{t("catalog.Price: High to Low")}</option>
            </select>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {products.map((product) => (
              <div key={product.id} className="p-3 rounded">
                ...
                <p className="text-md font-bold text-black">${product.price || "0"}</p>
